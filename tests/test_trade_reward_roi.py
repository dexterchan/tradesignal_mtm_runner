...
def get_trade_agent(pnl_config: PnlCalcConfig) -> TradeBookKeeperAgent:
    return TradeBookKeeperAgent(symbol="ETHUSD", pnl_config=pnl_config, fixed_unit=True)
...
def calculate_pnl_from_mtm_history(
    trade_book_keeper_agent: TradeBookKeeperAgent,
) -> float:
    mtm_history = trade_book_keeper_agent.mtm_history_value
    mtm_array = np.array(mtm_history)
    return mtm_array.sum()


@pytest.mark.skipif(
    "mtm_history_consistency_with_close_trade" not in test_cases, reason="skipped"
)
def test_mtm_history_consistency_with_close_trade(get_test_ascending_mkt_data) -> None:
    ...
    for i in range(0, len(test_mktdata)):
        ...
        if i == end_index:
            p1.close_position(
                exit_price=test_mktdata["close"][end_index],
    ...
    assert (
        calculate_pnl_from_mtm_history(trade_book_keeper_agent) - accumulated_pnl
    ) < COMPARE_ERROR
...
def test_roi_not_close_long_position(get_test_ascending_mkt_data) -> None:
    ...
    pnl_config = PnlCalcConfig.get_default()
    inx = 3
    pnl_config.roi = {inx: ((inx) * DATA_MOVEMENT + 1) / test_mktdata["close"][0]}
...
def test_roi_close_long_position(get_test_ascending_mkt_data) -> None:
    ...
    assert (
        trade_book_keeper_agent.archive_long_positions_list[0].close_reason
        == Proxy_Trade_Actions.ROI
    )
    assert len(trade_book_keeper_agent.outstanding_short_position_list) == 0
...
def test_roi_not_close_short_position(get_test_ascending_mkt_data) -> None:
    ...
    correct_pnl_normalized: float = p.calculate_pnl_normalized(
        test_mktdata["close"][-1]
    )
    assert (
        calculate_pnl_from_mtm_history(trade_book_keeper_agent) - correct_pnl_normalized
    ) < COMPARE_ERROR
