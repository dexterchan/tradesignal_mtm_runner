...
test_symbol = "ETHUSD"
test_cases: list = ["flat", "flat_close", "ascending"]
...
def test_trade_pnl_runner_with_flat_data_noclose(
    get_test_flat_mkt_data, get_pnl_config
) -> None:
    test_mktdata: pd.DataFrame = get_test_flat_mkt_data(
        dim=DATA_DIM, step=DATA_MOVEMENT
    )
    ...
    assert (
        trade_book_keeper_agent.outstanding_long_position_list[
            0
        ].calculate_pnl_normalized(test_mktdata["close"][0], True)
        == agent_mtm
    )
...
def test_trade_pnl_runner_with_flat_data_close(
    get_test_flat_mkt_data, get_pnl_config
) -> None:
    test_mktdata: pd.DataFrame = get_test_flat_mkt_data(
        dim=DATA_DIM, step=DATA_MOVEMENT
    ...
    assert (
        reduce(lambda x, y: x + y, trade_book_keeper_agent.mtm_history_value)
        == -FEE_RATE * 2
    )
...
def test_trade_pnl_runner_with_ascending_data_close(
    get_test_ascending_mkt_data, get_pnl_config
) -> None:
    ...
    pnl_config.roi = {
        ...
        inx: ((inx) * DATA_MOVEMENT - 1)
        / test_mktdata["close"][0]
    }
    ...
    agent_mtm: float = trade_book_keeper_agent.calculate_pnl_from_mtm_history()
    expected_pnl = (
        (test_mktdata["close"][inx] / test_mktdata["close"][0]) - 1 - 2 * FEE_RATE
    )
